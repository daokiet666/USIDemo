...
$query = "SELECT s.MaSuKien, s.TenSuKien, s.MoTa, s.ThoiGian, s.DiaDiem, d.TinhTrangXacNhan, d.Checkin, d.MaDangKy
FROM SuKien s
LEFT JOIN DangKy d ON s.MaSuKien = d.MaSuKien
WHERE MaStartup = $demo_user
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        if ($row['ThoiGian'] < date('Y-m-d')) {
            if ($row['Checkin'] == FALSE) {$status = 'Không tham gia';} 
            else {$status = 'Đã tham gia';}
        } else { $status = $row['TinhTrangXacNhan'];
        }
        $table_body .= "<tr>";
        ...
        if ($row['ThoiGian'] > date('Y-m-d') && $row['TinhTrangXacNhan'] == 'Chưa xác nhận') {
            $table_body .= '<td>
                        <button class="edit-btn" onclick="showPopUp(' . $row['MaDangKy'] . ')">Xác nhận</button>
                       </td>';
        }
        else {
            $table_body .= '<td>
                        <button class="view-btn" onclick="showPopUp(' . $row['MaDangKy'] . ')">Xem</button>
                       </td>';
        }
        $table_body .= "</tr>";
    }
...
<script>
    function showPopUp(id) {
        document.getElementById('php-frame').src = 'event.php?id='+id;
